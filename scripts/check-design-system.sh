...
# rive/                  a foreign artifact this repo only receives. See the
#                        block below — this one has three separate reasons and
#                        needs the room.
# transitA/              rollback snapshot of that same atlas. Same palette,
#                        same source, same "do not restyle here" rule.
...
#
# WHY rive/ IS EXEMPT FROM CHECK 1 AND NOT JUST FIXED
# -----------------------------------------------------
# The other two guard lists already ruled on the published atlas and this one
# did not, which is the whole bug. check-palette-v2.sh's EXCLUDE_RE carries it
...
#
# 1. Editing rive/ does not stick. publish-transit.yml rsyncs --delete from
#    LeBonhommePharma/Transit on a */15 cron as transit-publish[bot]. A link
#    added to rive/index.html is gone within the hour, and CI reports green
...
#    :root with the atlas's meanings: --gold is a sodium-lamp amber, not ΔG.
...
#    and repaints a live public-transit atlas. Note also that rive/index.html
...
#
# 3. The directory is the honest boundary. rive/ is exactly the tree
...
#    narrower while being worse: a third atlas page would land red on a bot
#    commit, for a reason no one could fix from here.
#    /transit/ is NOT this tree — it is an apex-authored 200 alias and stays
#    in scope. transitA/ is the rollback snapshot of rive/ and stays exempt.
#
# WHAT THIS EXEMPTION COSTS, STATED PLAINLY
# Nothing now checks rive/ styling. Not this guard, not check-palette-v2.sh,
...
# in the Transit repo, against public/Transit/, in the atlas's vocabulary. Until
# that exists, drift in the atlas is caught by eye. Do not widen this pattern
# past rive/ or transitA/ to make some other page quiet.
EXEMPT_RE='^(design/palette-v2/|rive/|transitA/|benchmark/|entropy-driven/)'
...
SCAN_EXCLUDE_RE='^(design/palette-v2/|rive/|transitA/|style\.css$|assets/index-[A-Za-z0-9_-]+\.css$|scripts/)'
